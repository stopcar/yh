...
<body>
		...
		<div id="zhongxin"  style="margin-top: 20px;">
			...
			<div class="control-group">
				...
				<c:choose>
		 			<c:when test="${pd.NAME==''|| pd.NAME==null }">
		 				<input type="file" name="NAME" id="NAME"   placeholder="这里输入名字" title="名字"/>
		 			</c:when>
		 			<c:otherwise>
		 			<a href="<%=basePath+Const.FILEPATHFILE%>${pd.NAME}" target="_blank">${pd.NAME}</a>
					<input type="button" class="btn btn-mini btn-danger" value="删除" onclick="delFile('${pd.NAME}','${pd.USER_ID}','NAME');" />
					<input type="hidden" name="NAME_HIDDEN" id="NAME_HIDDEN" value="${pd.NAME}"/>
		 			</c:otherwise>
		 		</c:choose>
				</div>
			</div>
			...
			<div class="control-group">
				...
				<c:choose>
					...
					<input type="button" class="btn btn-mini btn-danger" value="删除" onclick="delFile('${pd.IP}','${pd.USER_ID}','IP');" />
			...
			<div class="control-group">
				...
				<div class="controls">
					<textarea name="END_TIME" id="END_TIME" rows="5" class="form-control" placeholder="这里输入结束时间" title="结束时间">${pd.END_TIME}</textarea>
				</div>
			</div>
	...
	function delFile(fileColValue,USER_ID,fileCol){
		 if(confirm("确定要删除文件吗？")){
			var url = "sysAppUserCopy/delFile.do?"+fileCol+"="+fileColValue+"&USER_ID="+USER_ID+"&fileColName="+fileCol+"&guid="+new Date().getTime();
